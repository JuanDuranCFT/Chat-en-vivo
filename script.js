...
// 📥 Mensajes en tiempo real + edición
    ...
    snapshot.forEach(doc => {
        ...
        msg.innerHTML = `
            ...
            <div class="text" data-id="${doc.id}">
                ${data.text ? `<div class="editable">${data.text}</div>` : ""}
...
function enableEdit() {
    document.querySelectorAll(".editable").forEach(el => {
        el.ondblclick = async () => {
            const newText = prompt("Editar mensaje:", el.innerText);
            if (!newText || !newText.trim()) return;

            const id = el.closest(".text").dataset.id;
            await db.collection("mensajes").doc(id).update({ text: newText });
        };
    });
}
